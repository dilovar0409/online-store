                                ...
                                {cart.map(cart => (
                                    <div className="single-cart" key={cart._id}>
                                        ...
                                        <div className="counting">
                                            <button onClick={() => { props.decreaseItem(cart._id); props.getTotal() }}>-</button>
                                            <span>{cart.count}</span>
                                            <button onClick={() => { props.increaseItem(cart._id); props.getTotal() }}>+</button>
                                        </div>
                                        <div className="price">
                                            <h4>${cart.price * cart.count}</h4>
                                        </div>
                                        <div className="delete-item">
                                            <i className="fas fa-trash" onClick={() => { props.deleteItem(cart._id); props.getTotal() }}></i>
                                        </div>
                                    </div>
                                ))}
                            </div>
...
export default connect(mapStateToProps, { deleteItem, increaseItem, decreaseItem, getTotal })(Cart)
